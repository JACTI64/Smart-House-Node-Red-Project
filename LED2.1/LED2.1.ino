...
void testSequence() {
  const int totalTestDuration = 60000;  // 1 min total
  int perLedDuration = totalTestDuration / numLeds;
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "control" && message.equalsIgnoreCase("start")) {
    Serial.println("Start command received — running LED test sequence");
    runLedTest = true;
  }
}
